...
class EmailGroupActionsConverter {
    ...
    companion object {
        private val logger = LogManager.getLogger(EmailGroupActionsConverter::class)
        ...
        fun convertSearchEmailGroupRequestToGetNotificationConfigRequest(request: SearchEmailGroupRequest): GetNotificationConfigRequest {
            ...
            filterParams["config_type"] = ConfigType.EMAIL_GROUP.toString()

            val sortField = if (table.sortString == "email_group.name.keyword") NotificationConstants.NAME_TAG else null

            return GetNotificationConfigRequest(emptySet(), fromIndex, maxItems, sortField, sortOrder, filterParams)
        }

        fun convertGetNotificationConfigResponseToSearchEmailGroupResponse(
            response: GetNotificationConfigResponse?
        ): SearchEmailGroupResponse {
            if (response == null) throw OpenSearchStatusException("Email Group cannot be found.", RestStatus.NOT_FOUND)
            val searchResult = response.searchResult
            val emailGroups = mutableListOf<org.opensearch.alerting.model.destination.email.EmailGroup>()
            var invalidCount = 0
            searchResult.objectList.forEach {
                val emailGroup = convertNotificationConfigInfoToEmailGroup(it)
                if (emailGroup != null) {
                    emailGroups.add(emailGroup)
                } else {
                    logger.error("Email Group was null and cannot be converted")
                    invalidCount++
                }
            }
            return SearchEmailGroupResponse(RestStatus.OK, searchResult.totalHits.toInt() - invalidCount, emailGroups)
        }
        ...
        fun convertNotificationConfigInfoToEmailGroup(notificationConfigInfo: NotificationConfigInfo): org.opensearch.alerting.model.destination.email.EmailGroup? {
            val notificationConfig = notificationConfigInfo.notificationConfig
            if (notificationConfig.configType != ConfigType.EMAIL_GROUP) return null
            val emailGroup: EmailGroup = notificationConfig.configData as EmailGroup
            val recipients = mutableListOf<EmailEntry>()
            emailGroup.recipients.forEach {
                recipients.add(EmailEntry(it))
