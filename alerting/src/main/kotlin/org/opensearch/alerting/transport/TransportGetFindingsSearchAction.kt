...
class TransportGetFindingsSearchAction @Inject constructor(
                ...
                override fun onResponse(response: SearchResponse) {
                    ...
                    findings.forEach {
                        ...
                        for (docId in documentIds) {
                            documents["${it.index}|$docId"]?.let { document -> relatedDocs.add(document) }
                        }
    ...
    fun searchDocument(
        ...
        response.responses.forEach {
            ...
            val findingDocument = FindingDocument(it.index, it.id, !it.isFailed,  if (it.isFailed) emptyMap<String, Any>() else it.response.sourceAsMap)
